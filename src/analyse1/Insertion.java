...
public static String insertInTable(String query) {
		...
		if(requette[1].toUpperCase().equals("INTO") && query.contains("VALUES")) {
			if(keys.length==values.length) {
				...
				Champ.insert(bdd, table, keysValues);
                                message="Une ligne a été inserée avec succès";
                        
                        }
			else {
				message="Nombre colonne incompatible avec le Nombre de valeurs";
